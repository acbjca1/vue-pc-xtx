<template>
  ...
  <div class="sub-filter" v-if="!filterLoading">
    <div class="item">
      ...
      <div class="body">
        <a
          :class="{ active: item.id === filterData.brands.selecteBrand }"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    watch(() => route.params.id, (newVal) => {
      ...
      if (newVal && `/category/sub/${newVal}` === route.path) {
        ...
        findSubCategoryFilter(route.params.id).then(data => {
          ...
          // 1.品牌
          data.result.brands.selecteBrand = null
    ...
    const getFilterParams = () => {
      ...
      // 品牌
      obj.brandId = filterData.value.brands.selecteBrand
      // 销售属性
      ...
      // 参数数据：{brandId:'',attrs:[{groupName:'',propertyName:''},...]}
      return obj
    }
    // 1.记录当前选择的品牌
    const changeBrand = (brandId) => {
      if (filterData.value.brands.selecteBrand === brandId) return
      filterData.value.brands.selecteBrand = brandId
